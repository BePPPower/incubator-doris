...
NewJdbcScanner::NewJdbcScanner(RuntimeState* state, NewJdbcScanNode* parent, int64_t limit,
    ...
    _jdbc_connector_profile.reset(new RuntimeProfile("JdbcConnector"));
    get_parent()->_scanner_profile->add_child(_jdbc_connector_profile.get(), true, nullptr);
    _load_jar_timer = ADD_TIMER(_jdbc_connector_profile, "LoadJarTime");
    _init_connector_timer = ADD_TIMER(_jdbc_connector_profile, "InitConnectorTime");
    _check_type_timer = ADD_TIMER(_jdbc_connector_profile, "CheckTypeTime");
    _get_data_timer = ADD_TIMER(_jdbc_connector_profile, "GetDataTime");
}

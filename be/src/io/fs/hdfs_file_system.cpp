...
Status HdfsFileSystem::connect() {
    ...
    HdfsFsHandle* fs_handle = nullptr;
    RETURN_IF_ERROR(HdfsFsCache::instance()->get_connection(_hdfs_params, &fs_handle));
    if (!fs_handle) {
        return Status::InternalError("failed to init Hdfs handle with, please check hdfs params.");
    }
    _fs_handle.reset(fs_handle);
    return Status::OK();
}
...
std::shared_ptr<HdfsFsHandle> HdfsFileSystem::get_handle() const {
    std::lock_guard lock(_handle_mu);
    return _fs_handle;
}

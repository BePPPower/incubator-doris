...
public class JdbcPostgreSQLClientCachedClient extends JdbcClientCachedClient {
    ...
    public Table getTable(String dbName, String tblName) throws Exception {
        String sql = "SELECT \"TBL_NAME\", \"DBS\".\"NAME\", \"OWNER\", \"CREATE_TIME\","
                + " \"LAST_ACCESS_TIME\", \"LAST_ACCESS_TIME\", \"RETENTION\","
                + " \"IS_REWRITE_ENABLED\", \"VIEW_EXPANDED_TEXT\", \"VIEW_ORIGINAL_TEXT\", \"OWNER_TYPE\""
        ...
        try (Connection conn = getConnection();
            ...
            if (rs.next()) {
                ...
                hiveTable.setParameters(getParameters(dbName, tblName));
                hiveTable.setPartitionKeys(getTablePartitionKeys(dbName, tblName));
                return hiveTable;
            }
    ...
    private List<FieldSchema> getTablePartitionKeys(String dbName, String tblName) throws Exception {
        String sql = "SELECT \"PKEY_NAME\", \"PKEY_TYPE\", \"PKEY_COMMENT\" from \"PARTITION_KEYS\" WHERE \"TBL_ID\"=("
                + "SELECT \"TBL_ID\" FROM \"TBLS\" join \"DBS\" on \"TBLS\".\"DB_ID\" = \"DBS\".\"DB_ID\" "
                + "WHERE \"DBS\".\"NAME\" = '" + dbName + "' AND \"TBLS\".\"TBL_NAME\"='" + tblName + "'"
                + ");";
    ...
    private Map<String, String> getParameters(String dbName, String tblName) throws Exception {
        String sql = "SELECT \"PARAM_KEY\", \"PARAM_VALUE\" from \"TABLE_PARAMS\" WHERE \"TBL_ID\" = ("
                + "SELECT \"TBL_ID\" FROM \"TBLS\" join \"DBS\" on \"TBLS\".\"DB_ID\" = \"DBS\".\"DB_ID\""
                + " WHERE \"DBS\".\"NAME\" = '" + dbName + "' AND \"TBLS\".\"TBL_NAME\"='" + tblName + "'"
                + ");";

...
public class BackendsTableValuedFunction extends MetadataTableValuedFunction {
    ...
    public BackendsTableValuedFunction(Map<String, String> params) throws AnalysisException {
        if (params.size() > 1) {
            throw new AnalysisException("backends table-valued-function only support one param");
        }
        if (!params.containsKey(CLUSTER_NAME)) {
            throw new AnalysisException("backends table-valued-function only support cluster_name param");
        }
    }
